...
use crate::type_checker::environment::state::StateType;
use crate::type_checker::environment::Environment;
use crate::type_checker::infer::{infer, InferResult};
use crate::type_checker::type_result::TypeErr;

pub fn infer_optional(ast: &AST, env: &Environment, ctx: &Context) -> InferResult {
    match &ast.node {
        Node::Question { left, right } => {
            let (left_ty, env) =
                infer(left, &env.new_state(&env.state.as_state(StateType::Nullable)), ctx)?;
