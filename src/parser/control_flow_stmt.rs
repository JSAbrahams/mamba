...
fn parse_while(it: &mut Peekable<Iter<TokenPos>>, ind: i32) -> ParseResult<ASTNode> {
    ...
    let (cond, ind) = get_or_err!(it,parse_expression(it, ind), "while condition");
    check_next_is!(it, ind, Token::Do);
    let (expr_or_do, ind) = get_or_err!(it,parse_expr_or_stmt(it, ind), "while body");
...
fn parse_for(it: &mut Peekable<Iter<TokenPos>>, ind: i32) -> ParseResult<ASTNode> {
    ...
    let (expr, ind) = get_or_err!(it,parse_expression(it, ind), "for expression");
    check_next_is!(it, ind, Token::In);
    let (collection, ind) = get_or_err!(it,parse_expression(it, ind), "for collection");
    check_next_is!(it, ind, Token::Do);
    let (for_bod, ind) = get_or_err!(it,parse_expr_or_stmt(it, ind), "for body");
