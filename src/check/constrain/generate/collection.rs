...
pub fn gen_coll(
    ...
    match &ast.node {
        ...
        Node::Dict { elements } => {
            let elements: Vec<AST> =
                elements.iter().flat_map(|(from, to)| [from.clone(), to.clone()]).collect();
            gen_vec(&elements, env, false, ctx, constr)?;
        ...
        Node::DictBuilder { from, to, conditions } => {
            ...
            let set = AST::new(
                ast.pos,
                Node::Dict { elements: vec![(from.deref().clone(), to.deref().clone())] },
            );
    ...
    if let Some(cond) = conditions.first() {
        ...
        if let Some(conditions) = conditions.strip_prefix(&[cond.clone()]) {
            for cond in conditions {
                ...
                constr
                    .add_constr(&Constraint::truthy("comprehension condition", &cond), &conds_env);
            }
        }
...
pub fn constr_col_lookup(
    ...
    let next_access =
        Access { entity: Box::from(iter_ty), name: Box::new(Expected::new(exp_col.pos, &fun)) };

    let next_ty = Expected::new(exp_col.pos, &Type { name: col_ty.clone() });
    let next_constr =
        Constraint::new("iterable", &next_ty, &Expected::new(exp_col.pos, &next_access));
