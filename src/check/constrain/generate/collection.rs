    ...
    match &ast.node {
        ...
        Node::DictBuilder {
            ...
            let set = AST::new(
                ast.pos,
                Node::Dict {
                    elements: vec![(from.deref().clone(), to.deref().clone())],
                },
            );
    ...
    if let Some(cond) = conditions.first() {
        ...
        if let Some(conditions) = conditions.strip_prefix(&[cond.clone()]) {
            for cond in conditions {
                ...
                constr.add_constr(
                    &Constraint::truthy("comprehension condition", &cond),
                    &conds_env,
                );
            }
        }

        Ok(if env.is_def_mode {
            conds_env
        } else {
            env.clone()
        })
        ...
        Err(vec![TypeErr::new(
            ast.pos,
            "Builder must have a least one element",
        )])
    }
}
...
fn gen_col(collection: &AST, env: &Environment, constr: &mut ConstrBuilder) -> Constrained {
    let (col_ty, col_items_ty) = match &collection.node {
        ...
        Node::Dict { elements } => {
            ...
            let col_exp = Type {
                name: Name::from(&StringName::new(DICT, &[from_name, to_name])),
            };
        ...
        Node::Tuple { elements } => {
            ...
            let col_exp = Type {
                name: Name::from(&StringName::new(TUPLE, &names)),
            };
            let col_exp = Expected::new(collection.pos, &col_exp);
            constr.add("collection", &col_exp, &Expected::from(collection), env);
            return Ok(env.clone());
    ...
    let col_exp = Type {
        name: Name::from(&StringName::new(col_ty, &[col_items_ty])),
    };
    let col_exp = Expected::new(collection.pos, &col_exp);
    constr.add("collection", &col_exp, &Expected::from(collection), env);
    Ok(env.clone())
...
pub fn constr_col_lookup(
    ...
    let fun = Function {
        name: StringName::from(ITER),
        args: vec![exp_col.clone()],
    };
    let col_iterator = Expected::new(
        exp_col.pos,
        &Access {
    ...
    let next_ty = Expected::new(
        exp_col.pos,
        &Type {
            name: col_ty.clone(),
        },
    );
    let next_constr = Constraint::new(
        "iterable",
        &next_ty,
        &Expected::new(exp_col.pos, &next_access),
    );
