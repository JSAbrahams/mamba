...
pub fn unify_function(
    ...
    match (&left.expect, &right.expect) {
        (Function { args, .. }, Type { name }) | (Type { name }, Function { args, .. }) => {
            let arguments_union: Vec<Vec<Name>> =
                name.names.iter().cloned().map(|n| n.args(right.pos)).collect::<Result<_, _>>()?;
...
fn field_access(
    ...
    for entity_name in &entity_name.names {
        let field = ctx
            .class(entity_name, accessed.pos)
...
fn function_access(
    ...
    for entity_name in &entity_name.names {
        ...
        pushed +=
            unify_fun_arg(entity_name, name, &fun.arguments, args, constraints, accessed.pos)?;
    }
    ...
    for either_or_both in ctx_f_args.iter().zip_longest(args.iter()) {
        match either_or_both {
            EitherOrBoth::Both(ctx_f_arg, expected) => {
                ...
                let ctx_arg_ty =
                    Expected::new(expected.pos, &Type { name: arg_name.is_interchangeable(true) });

                // self is special, because self is equal to entity name
                let expected = if ctx_f_arg.name == SELF {
                    if let Type { name } = &expected.expect {
                        let entity_name = if ctx_f_arg.mutable {
                            entity_name.as_mutable()
                        } else {
                            entity_name.clone()
                        };
                        Expected::new(
                            expected.pos,
                            &Type { name: name.as_name(&entity_name, pos)? },
                        )
                    } else {
                        expected.clone()
                    }
                ...
                if let Ok(Ok(tuple_union)) = expected.ty().map(|name| name.elements(expected.pos)) {
                    ...
                    if name == &StringName::from(STR) {
                        for tuple_element in tuple_union.iter().flatten() {
                            let expected =
                                Expected::new(expected.pos, &Type { name: tuple_element.clone() });
            ...
            EitherOrBoth::Left(fun_arg) if !fun_arg.has_default => {
                let msg =
                    format!("Expected argument for '{fun_arg}' in method {name} of {entity_name}");
                return Err(vec![TypeErr::new(pos, &msg)]);
            }
