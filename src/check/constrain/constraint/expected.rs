...
use crate::check::context::clss::{BOOL_PRIMITIVE, FLOAT_PRIMITIVE, INT_PRIMITIVE, NONE,
                                  STRING_PRIMITIVE};
...
impl TryFrom<(&AST, &HashMap<String, String>)> for Expect {
    ...
    /// Also substitutes any identifiers with new ones from the environment if
    /// the environment has a mapping.
    /// This means that we forget about shadowed variables and continue with the
    /// new ones.
...
impl Expect {
    /// True if same value.
    ///
    /// If other is a Raises or Type where the Name is temporary, and either is
    /// Raises in former case or Type in latter, then also true.
    ...
    pub fn none() -> Expect { Expect::Type { name: Name::from(NONE) } }
