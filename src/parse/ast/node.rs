        ...
        let name = match &self {
            ...
            Node::FunctionCall { name, args } => {
                format!(
                    "{}({})",
                    name.node,
                    comma_delm(args.iter().map(|a| a.node.clone()))
                )
            }
            ...
            Node::Type { id, generics } => {
                if generics.is_empty() {
                    ...
                    format!(
                        "{}[{}]",
                        id.node,
                        comma_delm(generics.iter().map(|e| e.node.clone()))
                    )
                }
            }
            ...
            Node::Set { elements } => {
                format!(
                    "{{{}}}",
                    comma_delm(elements.iter().map(|e| e.node.clone()))
                )
            }
...
impl Node {
    ...
    pub fn map(&self, mapping: &dyn Fn(&Node) -> Node) -> Node {
        match mapping(self) {
            ...
            Node::Raise { error } => Node::Raise {
                error: Box::from(error.map(mapping)),
            },
            Node::Handle {
                expr_or_stmt,
                cases,
            ...
            Node::QuestionOp { expr } => Node::QuestionOp {
                expr: Box::from(expr.map(mapping)),
            },
...
mod test {
    ...
    fn call_same_value() {
        ...
        two_ast!(Node::FunctionCall {
            name: first.clone(),
            args: vec![*second.clone()]
        });
        two_ast!(Node::PropertyCall {
    ...
    fn type_equal_value() {
        ...
        two_ast!(Node::Condition {
            cond: first.clone(),
            el: Some(second.clone())
        });
    }
    ...
    fn collection_same_value() {
        ...
        two_ast!(Node::SetBuilder {
            item: item.clone(),
            conditions: vec![*item.clone()]
        });
        two_ast!(Node::ListBuilder {
            item: item.clone(),
            conditions: vec![*item.clone()]
        });
    }
    ...
    #[test]
    fn block_same_value() {
        let first = Box::from(AST::new(
            Position::invisible(),
            Node::Id {
                lit: String::from("asdf"),
            },
        ));
        let second = Box::from(AST::new(
            Position::invisible(),
            Node::Id {
    ...
    fn unary_op_same_value() {
        let expr = Box::from(AST::new(
            Position::invisible(),
            Node::Id {
                lit: String::from("qwerty"),
            },
        ));
    ...
    fn contrl_flow_same_value() {
        ...
        two_ast!(Node::Match {
            cond: cond.clone(),
            cases: vec![*body.clone(), *third.clone()]
        });
        two_ast!(Node::Case {
            cond: cond.clone(),
            body: body.clone()
