...
impl Node {
    ...
    ///
    /// If False, then it might still be an expression if for instance it is a
    /// function call. In such a case, the type checker must determine if it
    /// is an expression.
...
mod test {
    ...
    fn def_equal_value() {
        ...
        two_ast!(Node::FunDef {
            ...
            body:   Some(Box::from(AST::new(&Position::default(), Node::Raise {
                error: third.clone()
            })))
        });
    }
    ...
    fn type_equal_value() {
        ...
        two_ast!(Node::Type {
            id:       first.clone(),
            generics: vec![*second.clone(), *third.clone()]
        });
    ...
    fn collection_same_value() {
        let item =
            Box::from(AST::new(&Position::default(), Node::Id { lit: String::from("asdf") }));
    ...
    fn block_same_value() {
        let first =
            Box::from(AST::new(&Position::default(), Node::Id { lit: String::from("asdf") }));
        let second =
            Box::from(AST::new(&Position::default(), Node::Id { lit: String::from("lkjh") }));
    ...
    fn docstr_different_str_same_value() {
        two_ast!(Node::DocStr { lit: String::from("asdf") }, Node::DocStr {
            lit: String::from("lkjh")
        });
    }
    ...
    fn unary_op_same_value() {
        let expr =
            Box::from(AST::new(&Position::default(), Node::Id { lit: String::from("qwerty") }));
    ...
    fn question_equal_value() {
        two_ast!(Node::QuestionOp {
            expr: Box::from(AST::new(&Position::default(), Node::Continue))
        });
    ...
    fn comment_op_equal_value_different_string() {
        two_ast!(Node::Comment { comment: String::from("cca") }, Node::Comment {
            comment: String::from("aaa")
        });
    }
    ...
    #[test]
    fn empty_block_not_expression() { assert!(!Node::Block { statements: vec![] }.is_expression()) }
    ...
    fn operator_is_expression() {
        let left =
            Box::from(AST::new(&Position::default(), Node::Id { lit: String::from("left") }));
        let right =
            Box::from(AST::new(&Position::default(), Node::Id { lit: String::from("right") }));
