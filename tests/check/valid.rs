...
#[cfg_attr(feature = "imports", test_case("class", "import"))]
#[test_case("class", "trait_and_type")]
#[test_case("class", "top_level_tuple")]
#[test_case("class", "associated_function")]
#[test_case("class", "self_type")]
...
#[test_case("class", "pure_new_no_class_arguments")]
#[test_case("class", "new_underscore_one_argument")]
#[test_case("class", "new_rest_one_argument")]
#[test_case("class", "redundant_new")]
...
// Assigning a match to an unannotated variable leaves that variable's type unresolved, so any
// later use of it fails. `def a: Int := match ... end` is accepted. See tests/README.md.
#[test_case("control_flow", "match_wildcard_only" => ignore["a match assigned without an annotation leaves the variable untyped"])]
#[test_case("control_flow", "match_single_irrefutable_arm" => ignore["a match assigned without an annotation leaves the variable untyped"])]
#[test_case("control_flow", "match_subject_is_call" => ignore["a match assigned without an annotation leaves the variable untyped"])]
#[test_case("control_flow", "match_arm_body_block" => ignore["a match assigned without an annotation leaves the variable untyped"])]
#[test_case("control_flow", "match_shadow_restores_outer" => ignore["a capture pattern leaks out of its arm and clobbers the shadowed outer variable"])]
#[test_case("control_flow", "match_binding_read_in_arm" => ignore["a capture binding that shadows an outer name cannot be read in its arm"])]
#[test_case("control_flow", "match_guard_not_evaluated_when_pattern_fails" => ignore["a tuple case pattern does not bind into the guard or body"])]
#[test_case("control_flow", "match_tuple_pattern" => ignore["a tuple case pattern does not bind into the guard or body"])]
...
#[test_case("definition", "identifier_sqrt")]
...
#[test_case("function", "pure_calls_pure")]
#[test_case("function", "pure_method_calls_pure_method")]
#[test_case("function", "pure_method_calls_pure_function")]
#[test_case("function", "pure_reads_immutable")]
#[test_case("function", "pure_constructs")]
...
#[test_case("readme_example", "error_handling_desyntax_sugared" => ignore["a bare `match` on a fallible call is not checked as one, and a case cannot bind an error's constructor arguments yet"])]
...
#[test_case("readme_example", "error_handling")]
