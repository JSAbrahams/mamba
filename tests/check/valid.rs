...
#[test_case("readme_example", "error_handling_desyntax_sugared" => ignore["continues the class example, so m is not defined here, and a match case cannot bind an error's constructor arguments yet"])]
...
#[test_case("readme_example", "error_handling" => ignore["continues the class example, so Matrix2x2 is not defined here, and a match case cannot bind an error's constructor arguments yet"])]
