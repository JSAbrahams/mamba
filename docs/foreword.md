...
> A programming language should, ideally, make it easy to write clear, descriptive, unambiguous code, and difficult to write code that isn't those things
...
Each potential solution comes with its own set of both advantages and disadvantages.
...
There are a near-endless number of design choices, each of which comes with their own set of advantages and disadvantages.
...
- Static typing allows us to identify errors which would otherwise only be raised during execution, when it might already be too late.
...
There will always be something that a certain language, or tool, is not designed to solve.
It stands to reason that the computer scientists of the 70s and 80s couldn't have imagined that computers would become as ubiquitous as they are today.
Machine learning has taken a foothold in most major industries, and the internet of things is starting to take off.

I will say that certain language features, no matter how well intentioned they may have been, or insignificant they seemed at the time, may not have been the best choice in hindsight.
...
Certain features might allow very specific problems to be solved idiomatically, but at a point lines have to be drawn, or we run into some issues.
